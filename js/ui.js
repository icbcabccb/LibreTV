...
function addToViewingHistory(videoInfo) {
    ...
    try {
        ...
        // 保存到本地存储
        localStorage.setItem('viewingHistory', JSON.stringify(history));
        // 【新增】异步上传到 Cloudflare D1
    const userKey = localStorage.getItem('user_sync_key');
    if (userKey && videoInfo) {
        const payload = {
            video_id: videoInfo.vod_id || videoInfo.showIdentifier,
            source_code: videoInfo.sourceCode || 'unknown',
            title: videoInfo.title,
            cover: videoInfo.cover || '',
            url: videoInfo.url,
            episode_index: videoInfo.episodeIndex || 0,
            playback_position: videoInfo.playbackPosition || 0,
            duration: videoInfo.duration || 0
        };
        
        fetch(`/api/history?userKey=${encodeURIComponent(userKey)}`, {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify(payload)
        }).catch(e => console.log('Cloud sync err', e));
    }
    } catch (e) {
...
// ================= 云端同步功能 (Cloudflare D1) =================

// 获取/保存同步密钥
function getSyncKey() { return localStorage.getItem('user_sync_key') || ''; }

function setSyncKey() { 
    const keyInput = document.getElementById('userSyncKey');
    if (!keyInput) return;
    const key = keyInput.value.trim();
    
    if (key) {
        localStorage.setItem('user_sync_key', key);
        showToast('密钥已保存，正在同步...', 'success');
        syncCloudHistory(); 
    } else {
        localStorage.removeItem('user_sync_key');
        showToast('同步已关闭', 'info');
    }
}
...
async function syncCloudHistory() {
    const userKey = getSyncKey();
    if (!userKey) {
        showToast('请先在设置中保存同步密钥', 'warning');
        return;
    }

    const syncBtn = document.querySelector('button[onclick="syncCloudHistory()"]');
    const originalText = syncBtn ? syncBtn.innerText : '';
    if (syncBtn) syncBtn.innerText = '同步中...';

    try {
        const res = await fetch(`/api/history?userKey=${encodeURIComponent(userKey)}`);
        const result = await res.json();
        
        if (result.code === 200 && Array.isArray(result.list)) {
            let localHistory = [];
            try { localHistory = JSON.parse(localStorage.getItem('viewingHistory') || '[]'); } catch(e){}
            
            const mergedMap = new Map();
            
            // 本地数据入库
            localHistory.forEach(item => {
                const key = `${item.sourceCode}_${item.vod_id}`;
            ...
            // 云端数据合并（保留最新的）
            let hasUpdate = false;
            result.list.forEach(cloudItem => {
                const key = `${cloudItem.source_code}_${cloudItem.video_id}`;
                const localItem = mergedMap.get(key);

                if (!localItem || cloudItem.updated_at > localItem.timestamp) {
                    mergedMap.set(key, {
                        title: cloudItem.title,
                        cover: cloudItem.cover,
                        url: cloudItem.url,
                        sourceName: '云端同步', 
                        sourceCode: cloudItem.source_code,
                        vod_id: cloudItem.video_id,
                        episodeIndex: cloudItem.episode_index,
                        playbackPosition: cloudItem.playback_position,
                        duration: cloudItem.duration,
                        timestamp: cloudItem.updated_at,
                        episodes: localItem ? localItem.episodes : [] // 保护本地剧集列表
                    });
                    hasUpdate = true;
                }
            });
            ...
            if (hasUpdate) {
                ...
                localStorage.setItem('viewingHistory', JSON.stringify(mergedList));
                if (typeof loadViewingHistory === 'function') loadViewingHistory();
                showToast('同步成功', 'success');
            } else {
                showToast('记录已是最新', 'success');
            }
        }
    } catch (e) {
        ...
        if (syncBtn) syncBtn.innerText = originalText || '立即同步';
    }
}
