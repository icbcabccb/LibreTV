...
async function importConfigFromUrl() {
    ...
    document.getElementById('confirmUrlImport').addEventListener('click', async () => {
        ...
        try {
            ...
            if (config.name !== '司南搜索-Settings') throw '配置文件格式不正确';
...
async function exportConfig() {
    ...
    config['name'] = '司南搜索-Settings';  // 配置文件名，用于校验
    ...
    // 将配置数据保存为 JSON 文件
    saveStringAsFile(JSON.stringify(config), '司南搜索-Settings_' + times + '.json');
}
